...
class TSPEnv(_BaseEnv):
    ...
    def done(self) -> bool:
        ...
        if self._step_count == (self._node_num - 1):
            self._need_reset = True
            return True
        return False
    ...
    def reset(self, node_pos: torch.Tensor) -> TSPState:
        ...
        self._act_list = []
        self._act_list.append(torch.zeros((self.batch_size, 1), dtype=torch.long, device=self.device))
